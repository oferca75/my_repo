...
class Publicize_UI
{
        ...
        <div id="thickbox-content">
            ...
            <p style="text-align: center;">
                <input type="submit" value="<?php esc_attr_e('OK', 'jetpack') ?>"
                       class="button <?php echo $service_name; ?>-options save-options" name="save"
                       data-connection="<?php echo esc_attr($_REQUEST['connection']); ?>"
                       rel="<?php echo wp_create_nonce('save_' . $service_name . '_token_' . $_REQUEST['connection']) ?>"/>
            </p> <br/>
        </div>
        <?php
    }
    ...
    public static function global_checkbox($service_name, $id)
    {
        ...
        if (current_user_can($publicize->GLOBAL_CAP)) : ?>
            <p>
                <input id="globalize_<?php echo $service_name; ?>" type="checkbox" name="global"
                       value="<?php echo wp_create_nonce('publicize-globalize-' . $id) ?>"/>
                <label
                    for="globalize_<?php echo $service_name; ?>"><?php _e('Make this connection available to all users of this blog?', 'jetpack'); ?></label>
            </p>
        <?php endif;
    }
    ...
    function admin_page()
    {
        ...
        <p>&rarr; <a href="<?php echo esc_url($doc_link); ?>"
            ...
            <div class='left'>
                ...
                foreach ($services as $name => $service) :
                    ...
                    <div class="publicize-service-entry"
                         <?php if ($service_num > 0): ?>class="connected"<?php endif; ?> >
                        <div id="<?php echo esc_attr($name); ?>" class="publicize-service-left">
                            <a href="<?php echo esc_url($connect_url); ?>"
                               id="service-link-<?php echo esc_attr($name); ?>"
                               target="_top"><?php echo $this->publicize->get_service_label($name); ?></a>
                        </div>
                        ...
                        <div class="publicize-service-right">
                            <?php if ($this->publicize->is_enabled($name) && $connections = $this->publicize->get_connections($name)) : ?>
                                <ul>
                                    ...
                                    <?php endif; ?>
                                        ...
                                        <li class="publicize-connection"
                                            data-connection-id="<?php echo esc_attr($id); ?>">
                                            <?php esc_html_e('Connected as:', 'jetpack'); ?>
                                            <?php
                                            if (!empty($profile_link)) : ?>
                                            <a class="publicize-profile-link"
                                               href="<?php echo esc_url($profile_link); ?>" target="_top">
                                            ...
                                            <?php if (0 == $cmeta['connection_data']['user_id']) : ?>
                                                ...
                                                <?php if (current_user_can($this->publicize->GLOBAL_CAP)) : ?>
                                                    <a class="pub-disconnect-button"
                                                       title="<?php esc_html_e('Disconnect', 'jetpack'); ?>"
                                            ...
                                            <?php else : ?>
                                                <a class="pub-disconnect-button"
                                                   title="<?php esc_html_e('Disconnect', 'jetpack'); ?>"
                                                   href="<?php echo esc_url($disconnect_url); ?>" target="_top">×</a>
                                            <?php endif; ?>

                                            <br/>
                                            <div class="pub-connection-test test-in-progress"
                                                 id="pub-connection-test-<?php echo esc_attr($id); ?>">
                                            </div>
                                        </li>
                            ...
                            if (empty ($connections)) { ?>
                                <a id="<?php echo esc_attr($name); ?>" class="publicize-add-connection button"
                                   href="<?php echo esc_url($connect_url); ?>"
                                   target="_top"><?php echo esc_html(__('Connect', 'jetpack')); ?></a>
                            <?php } else { ?>
                                <a id="<?php echo esc_attr($name); ?>" class="publicize-add-connection button add-new"
                                   href="<?php echo esc_url($connect_url); ?>"
                                   target="_top"><?php echo esc_html(__('Add New', 'jetpack')); ?></a>
                            <?php } ?>
                        </div>
                    </div>
                <?php endforeach; ?>
        ...
        <div id="publicize" class="misc-pub-section misc-pub-section-last">
            ...
            if (0 < count($services)) :
                ...
                <div id="publicize-form" class="hide-if-js">
                            ...
                            foreach ($connections as $connection) {
                                ...
                                <li>
                                    <label for="wpas-submit-<?php echo esc_attr($unique_id); ?>">
                                        <input type="checkbox" name="wpas[submit][<?php echo $unique_id; ?>]"
                                               id="wpas-submit-<?php echo $unique_id; ?>"
                                               class="wpas-submit-<?php echo $name; ?>" value="1" <?php
                    ...
                    <textarea name="wpas_title"
                              id="wpas-title"<?php disabled($all_done); ?>><?php echo $title; ?></textarea>
                ...
                <div id="publicize-form" class="hide-if-js">
                    ...
                    <ul class="not-connected">
                        <?php foreach ($available_services as $service_name => $service) : ?>
                            <li>
                                <a class="pub-service" data-service="<?php echo esc_attr($service_name); ?>"
                                   title="<?php echo esc_attr(sprintf(__('Connect and share your posts on %s', 'jetpack'), $this->publicize->get_service_label($service_name))); ?>"
                                   target="_blank" href="<?php echo $this->publicize->connect_url($service_name); ?>">
                    ...
                    <?php if (0 < count($services)) : ?>
                        <a href="#" class="hide-if-no-js" id="publicize-form-hide"><?php _e('Hide', 'jetpack'); ?></a>
                    <?php else : ?>
                        <a href="#" class="hide-if-no-js"
                           id="publicize-disconnected-form-hide"><?php _e('Hide', 'jetpack'); ?></a>
                    <?php endif; ?>
            ...
            <span
                id="publicize-defaults"><strong><?php echo join('</strong>, <strong>', array_map('esc_html', $active)); ?></strong></span><br/>

            <?php if (0 < count($services)) : ?>
                <a href="#" id="publicize-form-edit"><?php _e('Edit Details', 'jetpack'); ?></a>&nbsp;<a
                    href="<?php echo admin_url('options-general.php?page=sharing'); ?>"
                    target="_blank"><?php _e('Settings', 'jetpack'); ?></a><br/>
            <?php else : ?>
