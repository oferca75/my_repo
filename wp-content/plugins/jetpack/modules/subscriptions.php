...
class Jetpack_Subscriptions
{
    ...
    function subscription_post_page_metabox()
    {
        ...
        if (get_post_status($post->ID) !== 'publish' && get_post_type($post->ID) == 'post') : ?>
            <div class="misc-pub-section">
                <label
                    for="_jetpack_dont_email_post_to_subs"><?php _e('Jetpack Subscriptions:', 'jetpack'); ?></label><br>
                <input type="checkbox" name="_jetpack_dont_email_post_to_subs" id="jetpack-per-post-subscribe"
                       value="1" <?php checked($disable_subscribe_value, 1, true); ?> />
    ...
    function subscription_post_subscribe_setting()
    {
        ...
        <p class="description">
            <input type="checkbox" name="stb_enabled" id="jetpack-post-subscribe"
                   value="1" <?php checked($stb_enabled, 1); ?> />
    ...
    function subscription_comment_subscribe_setting()
    {
        ...
        <p class="description">
            <input type="checkbox" name="stc_enabled" id="jetpack-comment-subscribe"
                   value="1" <?php checked($stc_enabled, 1); ?> />
    ...
    function widget($args, $instance)
    {
        ...
        if (isset($_GET['subscribe'])) :
            switch ($_GET['subscribe']) :
                ...
                case 'success' : ?>
                    <div
                        class="success"><?php echo wpautop(str_replace('[total-subscribers]', number_format_i18n($subscribers_total['value']), $success_message)); ?></div>
                    <?php break;
                default : ?>
        ...
        if (isset($_GET['subscribe']) && 'success' == $_GET['subscribe']) { ?>
            ...
            <form action="#" method="post" accept-charset="utf-8" id="subscribe-blog-<?php echo $widget_id; ?>">
                ...
                if (!isset ($_GET['subscribe']) || 'success' != $_GET['subscribe']) {
                    ?>
                    <div
                        id="subscribe-text"><?php echo wpautop(str_replace('[total-subscribers]', number_format_i18n($subscribers_total['value']), $subscribe_text)); ?></div><?php
                }
    ...
    function form($instance)
    {
        ...
        <p>
            <label for="<?php echo $this->get_field_id('success_message'); ?>">
                ...
                <textarea style="width: 95%" id="<?php echo $this->get_field_id('success_message'); ?>"
                          name="<?php echo $this->get_field_name('success_message'); ?>"
                          type="text"><?php echo esc_html($success_message); ?></textarea>
            </label>
        </p>
        <p>
