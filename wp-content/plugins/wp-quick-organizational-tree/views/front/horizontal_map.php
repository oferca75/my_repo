...
<style type="text/css">
    .first_name {
        color: # <?php echo $map['mp_title_color'] ?>;
    }

    .first_name a:link, .first_name a:hover, .first_name a:visited, .first_name a:active {
        color: # <?php echo $map['mp_title_color'] ?>;
        text-decoration: none;
    }

    ul.tree li > div {
        background: # <?php echo $map['mp_bgcolor'] ?>;
        max-width: <?php echo $maxWidth ?>px !important;
    }

    ul.tree li div.current {
        background: # <?php echo $map['mp_current_bgcolor'] ?>;
    }

    ul.tree li div.children {
        background: # <?php echo $map['mp_child_bgcolor'] ?>;
    }

    ul.tree li div.parent {
        background: # <?php echo $map['mp_parent_bgcolor'] ?>;
    }
</style>
<div class="wm-tree-container"
     style="padding: 0px; margin: 0px; background-color: #<?php echo $map['mp_container_bgc'] ?>;"
     id="treeContainer_<?php echo $uniqid ?>"><?php echo wm_horizontal_tree($mp_id, false) ?></div>
