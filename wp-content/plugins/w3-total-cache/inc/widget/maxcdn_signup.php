...
<div id="maxcdn-widget" class="sign-up maxcdn-netdna-widget-base">
    ...
    <?php if (!$authorized): ?>
        ...
        <a class="button-primary"
           href="<?php echo wp_nonce_url(w3_admin_url('admin.php?page=w3tc_dashboard&w3tc_cdn_maxcdn_signup'), 'w3tc') ?>"
           target="_blank"><?php _e('Sign Up Now and Save 25%', 'w3-total-cache') ?></a>
    ...
    <form action="admin.php?page=w3tc_dashboard" method="post">
        <p>
            ...
            <input id="cdn_maxcdn_authorization_key" class="w3tc-ignore-change"
                   type="text" <?php echo $is_sealed ? 'disabled="disabled"' : '' ?> name="cdn.maxcdn.authorization_key"
    ...
    <p id="create_zone_area"
       style="<?php echo $authorized && (!$have_zone || is_null($zone_info)) ? '' : 'display: none' ?>"><input
            type="button" class="button" onclick="w3tc_create_zone('maxcdn','<?php echo wp_create_nonce('w3tc'); ?>')"
            value="<?php _e('Create Default Zone', 'w3-total-cache') ?>"/>
        <span id="create_pull_zone_result" style="margin-left:15px;padding:3px 8px 3px 8px;"></span>
        <br/><span
            class="description"><?php _e('You have no zone connected with this site. Click button to create a default zone.', 'w3-total-cache') ?></span>
    </p>
    ...
    <?php if ($authorized && (!$have_zone || is_null($zone_info))): ?>
        <button id="netdna-maxcdn-create-pull-zone"
                class="button-primary {type: 'maxcdn', nonce: '<?php echo wp_create_nonce('w3tc'); ?>'}"><?php _e('Create Pull Zone', 'w3-total-cache') ?></button>
    <?php endif ?>
</div>
