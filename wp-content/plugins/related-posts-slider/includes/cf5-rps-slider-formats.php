...
function cf5_rps_wp_head_default()
{
    ...
    if ($cf5_rps['stylesheet'] == 'default'):
        ?>
        <style type="text/css">.rps_sldrtitle {
                font-family: <?php echo $cf5_rps['stitle_font'];?>;
                font-size: <?php echo $cf5_rps['stitle_size'];?>px;
                font-weight: <?php echo $cf5_rps['stitle_weight'];?>;
                font-style: <?php echo $cf5_rps['stitle_style'];?>;
                <?php if($stitle_color and !empty($stitle_color)){?> color: <?php echo $cf5_rps['stitle_color'];?>;
            <?php } ?>
            }

            .rps_wrapper {
                height: <?php echo $cf5_rps['height'];?>px;
                <?php if($bgcolor and !empty($bgcolor)){?> background: <?php echo $cf5_rps['bgcolor'];?>;
                <?php } ?><?php if($pcontent_color and !empty($pcontent_color)){?> color: <?php echo $cf5_rps['pcontent_color'];?>;
                <?php } ?> border: <?php echo $cf5_rps['obrwidth'];?>px solid <?php echo $cf5_rps['obrcolor'];?>;
                font-family: <?php echo $cf5_rps['pcontent_font'];?>;
                font-size: <?php echo $cf5_rps['pcontent_size'];?>px;
                line-height: <?php echo ($cf5_rps['pcontent_size']+4);?>px;
            }

            .rps_wrapper div.h1div {
                font-size: <?php echo $cf5_rps['ptitle_size'];?>px;
                line-height: <?php echo ($cf5_rps['ptitle_size']+4);?>px;
                font-family: <?php echo $cf5_rps['ptitle_font'];?>;
                font-weight: <?php echo $cf5_rps['ptitle_weight'];?>;
                font-style: <?php echo $cf5_rps['ptitle_style'];?>;
                border-bottom: <?php echo $cf5_rps['ibrwidth'];?>px solid <?php echo $cf5_rps['ibrcolor'];?>;
            }

            .rps_wrapper div.h1div a {
                <?php if($ptitle_color and !empty($ptitle_color)){?> color: <?php echo $cf5_rps['ptitle_color'];?> !important;
            <?php } ?>
            }

            .rps_wrapper div.h2div {
                font-family: <?php echo $cf5_rps['ltitle_font'];?>;
                font-size: <?php echo $cf5_rps['ltitle_size'];?>px;
                font-weight: <?php echo $cf5_rps['ltitle_weight'];?>;
                font-style: <?php echo $cf5_rps['ltitle_style'];?>;
                <?php if($ltitle_color and !empty($ltitle_color)){?> color: <?php echo $cf5_rps['ltitle_color'];?>;
                <?php } ?> line-height: <?php echo ($cf5_rps['ltitle_size']+4);?>px;
            }

            .rps_content {
                border: <?php echo $cf5_rps['ibrwidth'];?>px solid <?php echo $cf5_rps['ibrcolor'];?>;
                top: <?php echo ($cf5_rps['height']+10);?>px;
                <?php if($fgcolor and !empty($fgcolor)){?> background-color: <?php echo $cf5_rps['fgcolor'];?>;
            <?php } ?>
            }

            img.rps_thumb {
                width: <?php echo $cf5_rps['img_width'];?>% !important;
                max-height: <?php echo $cf5_rps['img_height'];?>px !important;
            <?php if($cf5_rps['img_align']=='left') {echo 'float:left;margin:0 5px 5px 0 !important;';}if($cf5_rps['img_align']=='right') {echo 'float:right;margin:0 0 5px 5px !important;';}	?>
            }

            .rps_content div.pdiv {
                border-top: <?php echo $cf5_rps['obrwidth'];?>px solid <?php echo $cf5_rps['obrcolor'];?>;
            }

            a.rps_more {
                <?php if($hvtext_color and !empty($hvtext_color)){?> color: <?php echo $cf5_rps['hvtext_color'];?> !important;
                <?php } ?> border: <?php echo $cf5_rps['ibrwidth'];?>px solid <?php echo $cf5_rps['ibrcolor'];?>;
                <?php if($hvcolor and !empty($hvcolor)){?> background-color: <?php echo $cf5_rps['hvcolor'];?>;
            <?php } ?>
            }

            .rps_item {
                border: <?php echo $cf5_rps['ibrwidth'];?>px solid <?php echo $cf5_rps['ibrcolor'];?>;
                <?php if($fgcolor and !empty($fgcolor)){?> background: <?php echo $cf5_rps['fgcolor'];?>;
            <?php } ?>
            }

            .rps_item:hover div.h2div, .rps_list .selected div.h2div, .rps_item:active div.h2div {
                <?php if($hvtext_color and !empty($hvtext_color)){?> color: <?php echo $cf5_rps['hvtext_color'];?>;
            <?php } ?>
            }

            .rps_item:hover, .selected {
                <?php if($hvcolor and !empty($hvcolor)){?> border-color: <?php echo $cf5_rps['hvcolor'];?>;
...
function cf5_rps_default($echo = true, $rps_posts = array())
{
    ...
    if ($rps_posts and !$rps_slider_shown):
        ...
        foreach ($rps_posts as $post_id) {
            ...
            if ($flag == 1) {
                $pcontent = cf5_rps_word_limiter($pcontent, $limit = $content_limit);
            }
        ...
        foreach ($rps_posts as $post_id) {
            ...
            if ($i % $cf5_rps['per_page'] == 0) {
                if ($i == 0) {
                    $page_html = '<div class="rps_page" style="display:block;">';
                    $selected = 'selected';
                } else {
                    $page_html = '<div class="rps_page">';
                }
            }
            if ($i % $cf5_rps['per_page'] == ($cf5_rps['per_page'] - 1)) {
                $page_close = '</div>';
            }
        ...
        if ($page_close == '' or empty($page_close)) {
            $slider = $slider . '</div>';
        }
...
function cf5_rps_wp_head_h_carousel()
{
    ...
    if ($cf5_rps['format_style'] == 'default'):
        ...
        <style type="text/css">.rps_sldrtitle {
            ...
            .rps_wrapper {
                <?php if($bgcolor and !empty($bgcolor)){?> background: <?php echo $cf5_rps['bgcolor'];?>;
                <?php } ?><?php if($ltitle_color and !empty($ltitle_color)){?> color: <?php echo $cf5_rps['ltitle_color'];?>;
                <?php } ?> border: <?php echo $cf5_rps['obrwidth'];?>px solid <?php echo $cf5_rps['obrcolor'];?>;
                font-family: <?php echo $cf5_rps['ltitle_font'];?>;
                font-size: <?php echo $cf5_rps['ltitle_size'];?>px;
                line-height: <?php echo ($cf5_rps['ltitle_size']+4);?>px;
            }

            img.rps_thumb {
                width: <?php echo $cf5_rps['img_width'];?>px !important;
                height: <?php echo $cf5_rps['img_height'];?>px !important;
                border: <?php echo $cf5_rps['ibrwidth'];?>px solid <?php echo $cf5_rps['ibrcolor'];?>;
            <?php if($cf5_rps['img_align']=='left') {echo 'float:left;margin:0 5px 5px 0 !important;';}if($cf5_rps['img_align']=='right') {echo 'float:right;margin:0 0 5px 5px !important;';}	?>
            }
            ...
            .rps_item {
                <?php if($fgcolor and !empty($fgcolor)){?> background: <?php echo $cf5_rps['fgcolor'];?>;
                <?php } ?> width: <?php echo $cf5_rps['hwidth'];?>px;
                height: <?php echo $cf5_rps['height'];?>px;
            }

            .rps_item a {
                <?php if($ltitle_color and !empty($ltitle_color)){?> color: <?php echo $cf5_rps['ltitle_color'];}?> !important;
            }
