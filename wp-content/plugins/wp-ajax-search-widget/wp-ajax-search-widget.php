...
class wpasw_widget extends WP_Widget
{
    ...
    function form($instance)
    {
        ...
        ?>
        <p class="wpasw-title"><label
                for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title', 'wpasw'); ?>: <input
                    class="widefat" id="<?php echo $this->get_field_id('title'); ?>"
                    name="<?php echo $this->get_field_name('title'); ?>" type="text"
                    value="<?php echo esc_attr($title); ?>"/></label></p>
        <p class="wpasw-result-limit"><label
                for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Results Limit', 'wpasw'); ?>
                :</label>
            ...
            if ($search->have_posts()) :
                        ...
                        <li <?php post_class(); ?>>
                            ...
                            <div class="entry-date">
                                <time class="published"
                                      datetime="<?php the_time('Y-m-d\TH:i:s') ?>"><?php the_date(); ?></time>
                            </div>
                        </li>
                        <?php
                    }
                ...
                if ($search->max_num_pages > 1) {
                    ...
                    if (locate_template('parts/widget-ajax-search-more.php') != '') {
                        ...
                        ?>
                        <li class="wpasw-more-link"><a
                                href="<?php echo esc_url(add_query_arg(array('s' => $s), home_url())); ?>"><?php _e('View all search results &hellip;', 'wpasw'); ?></a>
                        </li>
                        <?php
                    }
                }
