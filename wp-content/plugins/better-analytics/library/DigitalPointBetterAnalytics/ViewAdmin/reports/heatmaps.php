...
<h3 class="nav-tab-wrapper">
    <a class="nav-tab nav-tab-active"
       href="<?php echo menu_page_url('better-analytics_heatmaps', false) . (!empty($_REQUEST['page_path']) ? '&page_path=' . urlencode($_REQUEST['page_path']) : ''); ?>"><?php esc_html_e('Weekly Heat Maps', 'better-analytics'); ?></a>
    <a class="nav-tab"
       href="<?php echo menu_page_url('better-analytics_areacharts', false) . (!empty($_REQUEST['page_path']) ? '&page_path=' . urlencode($_REQUEST['page_path']) : ''); ?>"><?php esc_html_e('Charts', 'better-analytics'); ?></a>
    <a class="nav-tab"
       href="<?php echo menu_page_url('better-analytics_events', false) . (!empty($_REQUEST['page_path']) ? '&page_path=' . urlencode($_REQUEST['page_path']) : ''); ?>"><?php esc_html_e('Events', 'better-analytics'); ?></a>
    <a class="nav-tab"
       href="<?php echo menu_page_url('better-analytics_monitor', false) . (!empty($_REQUEST['page_path']) ? '&page_path=' . urlencode($_REQUEST['page_path']) : ''); ?>"><?php esc_html_e('Issue Monitoring', 'better-analytics'); ?></a>
</h3>
...
<form>
    <table id="parameters" class="form-table">
        ...
        <tr>
            <th></th>
            <td>
                <input type="number" name="weeks" id="ba_weeks" size="5" min="1" max="1000" step="1" value="4"/>
                &nbsp; <?php esc_html_e('Weeks Of Data, Ending', 'better-analytics') ?> &nbsp;
                <input type="number" name="end" id="ba_end" size="5" min="0" max="10000" step="1" value="1"/>
                &nbsp; <?php esc_html_e('Days Ago', 'better-analytics') ?>
            </td>
        </tr>
