...
class FoldingCategoryList extends WP_Widget
{
    ...
    function form($instance)
    {
        ...
        ?>
        <p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'focal'); ?> <input
                    class="widefat" id="<?php echo $this->get_field_id('title'); ?>"
                    name="<?php echo $this->get_field_name('title'); ?>" type="text"
                    value="<?php echo esc_attr($title); ?>"/></label></p>
        <p>
            <label for="<?php echo $this->get_field_id('definition'); ?>"><?php _e('Setting Definition:', 'focal'); ?>
                <select id="<?php echo $this->get_field_id('definition'); ?>"
                        name="<?php echo $this->get_field_name('definition'); ?>"><?php echo $options; ?></select></label>
        </p>
        <p>
            <small><?php _e('Please goto the <a href="/wp-admin/options-general.php?page=folding-category-widget/focal_wp28.php">settings page</a> to configure definitions.', 'focal'); ?></small>
        </p>
        <?php
    }
                ...
                <form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
                    ...
                    <table class="form-table">
                        <tr>
                            ...
                            <td>
                                <input type="checkbox" id="focal_UseCache"
                                       name="focal_UseCache" <?php echo ($this->defaultOptions['UseCache']) ? "checked" : ''; ?>/><br/>
                                ...
                                <input type="submit" name="focalRebuildCache"
                                       value="<?php _e('Rebuild Cache', 'focal'); ?>" class="button"/> &nbsp; <input
                                    type="submit" name="focalDeleteCache"
                                    value="<?php _e('Delete Cache', 'focal'); ?>" class="button"/> &nbsp; <input
                                    type="submit" name="focalUninstall"
                                    value="<?php _e('Uninstall/Reset', 'focal'); ?>" class="button"/>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row" valign="top"><?php _e('Auto Rebuild Cache', 'focal'); ?></th>
                            <td>
                                <input type="checkbox" id="focal_AutoRebuildCache"
                                       name="focal_AutoRebuildCache" <?php echo ($this->defaultOptions['AutoBuildCache']) ? "checked" : ''; ?>/><br/>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="checkbox" id="focal_UseDescriptions"
                                       name="focal_UseDescriptions" <?php echo ($this->defaultOptions['DontUseCategoryDescription']) ? '' : "checked"; ?>/><br/>
                    ...
                    <br/>
                    <span style="float:right"><input type="submit" name="focalSubmitOptions"
                                                     value="<?php _e('Save Changes', 'focal'); ?>"
                                                     class="button"/></span>
                    ...
                    <table class="form-table">
                        <tr>
                            ...
                            <td>
                                <select id="focalNumber" name="focalNumber"><?php echo $options; ?></select> &nbsp;
                                <input type="submit" name="focalChangeWidget"
                                       value="<?php _e('Load Definition', 'focal'); ?>" class="button"/>
                                &nbsp; <input type="submit" name="focalDeleteSetting"
                                              value="<?php _e('Delete Definition', 'focal'); ?>" class="button"/>
                                &nbsp; <input type="text" name="focalNewDefinition" value=''/> &nbsp; <input
                                    type="submit" name="focalNewDefinitionSubmit"
                                    value="<?php _e('New Definition', 'focal'); ?>" class="button"/>
                                <br/>
                                <input type="hidden" name="focal_selectedWidget"
                                       value="<?php echo $selectedWidget; ?>"/>
                                <span><?php _e('Select a definition from the list. Load or delete it, or enter a name and create a new definition.', 'focal'); ?>
                                    .</span>
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <label><input type='radio' name='focal_IncExCategories'
                                              value='ex' <?php if ($this->widgetOptions['IncOrEx'] == 'ex') echo ' checked'; ?>>
                                    <span
                                        class="setting-description"><?php _e('The categories ticked below will be <strong>Excluded</strong> from FoCaL.', 'focal'); ?></span></label><br/>
                                <label><input type='radio' name='focal_IncExCategories'
                                              value='inc'<?php if ($this->widgetOptions['IncOrEx'] == 'inc') echo ' checked'; ?>>
                                    <span
                                        class="setting-description"><?php _e('Only the categories ticked below will be <strong>Included</strong> in FoCaL.', 'focal'); ?></span></label><br/><br/>
                                <?php
                                if (function_exists('wp_category_checklist')) { ?>
                                    <div id="categorydiv"
                                         style="height:170px; width:220px; overflow:auto; background-color:white;padding:8px;border:1px solid #dfdfdf;">
                                ...
                                <?php } else {
                                    ?>
                                    <input type="text" name="focal_widgetExcluded" size="20"
                                           value="<?php echo implode(",", $this->widgetOptions['ExcludeCats']) ?>"/>
                                    <br/>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <select name="focal_widgetShowMethod" id="focal_widgetShowMethod"
                                        value="<?php echo $value; ?>">
                                    <option
                                        value="normal" <?php echo $normal ?>><?php _e('Normal (default)', 'focal'); ?></option>
                                    <option
                                        value="members" <?php echo $member ?>><?php _e('Expand all post categories', 'focal'); ?></option>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" name="focal_widegtPriority" id="focal_widegtPriority" size="40"
                                       value="<?php echo $this->widgetOptions['HightlightPriority'] ?>"/><br/>
                        ...
                        <tr>
                            ...
                            <td><?php
                                ...
                                if ($this->widgetOptions['OrderBy'] == 'name') {
                                    $name = ' selected';
                                    $value = 'name';
                                }
                                if ($this->widgetOptions['OrderBy'] == 'id') {
                                    $id = ' selected';
                                    $value = 'id';
                                }
                                if ($this->widgetOptions['OrderBy'] == 'description') {
                                    $description = ' selected';
                                    $value = 'description';
                                }
                                if ($this->widgetOptions['OrderBy'] == 'slug') {
                                    $slug = ' selected';
                                    $value = '';
                                }
                                if ($this->widgetOptions['OrderBy'] == 'postcount') {
                                    $postcount = ' selected';
                                    $value = 'postcount';
                                }
                                if ($this->widgetOptions['OrderBy'] == 'mycategoryorder') {
                                    $mycategoryorder = ' selected';
                                    $value = 'mycategoryorder';
                                }
                                if ($this->widgetOptions['OrderBy'] == 'titlelength') {
                                    $titlelength = ' selected';
                                    $value = 'titlelength';
                                }
                                if ($this->widgetOptions['OrderByDirection'] == 'asc') {
                                    $asc = ' selected';
                                    $valuea = 'asc';
                                }
                                if ($this->widgetOptions['OrderByDirection'] == 'desc') {
                                    $desc = ' selected';
                                    $valuea = 'desc';
                                }
                                ...
                                <select name="focal_widgetSortBy" id="focal_widgetSortBy" value="<?php echo $value; ?>">
                                    <option<?php echo $name; ?>
                                        value="name"><?php _e('Category Name (default)', 'focal'); ?></option>
                                    <option<?php echo $titlelength; ?>
                                        value="titlelength"><?php _e('Category Name Length', 'focal'); ?></option>
                                    <option<?php echo $id; ?>
                                        value="id"><?php _e('Category ID', 'focal'); ?></option>
                                    <option<?php echo $description; ?>
                                        value="description"><?php _e('Category Description', 'focal'); ?></option>
                                    <option<?php echo $slug; ?>
                                        value="slug"><?php _e('Category Slug', 'focal'); ?></option>
                                    <option<?php echo $postcount; ?>
                                        value="postcount"><?php _e('Category Post Count', 'focal'); ?></option>
                                    <?php if (function_exists("mycategoryorder_init")) { ?>
                                        <option<?php echo $mycategoryorder; ?>
                                        value="mycategoryorder"><?php _e('My Category Order', 'focal'); ?></option><?php } ?>
                                </select> &nbsp; <select name="focal_widgetSortBySeq" id="focal_widgetSortBySeq"
                                                         value="<?php echo $valuea; ?>">
                                    <option<?php echo $asc; ?>
                                        value="asc"><?php _e('Ascending (default)', 'focal'); ?></option>
                                    <option<?php echo $desc; ?>
                                        value="desc"><?php _e('Descending', 'focal'); ?></option>
                                </select><br/>
                                <span><?php _e('How should the list be sorted?', 'focal'); ?> <?php if (function_exists("mycategoryorder_init")) { ?>
                                        <em><?php _e('nb: if selecting My Category Order you must manually rebuild the cache after changing the order.', 'focal'); ?></em><?php } ?></span>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row" valign="top"><?php _e('Post Count', 'focal'); ?></th>
                            <td>
                                <input type="checkbox" id="focal_ShowPostCount"
                                       name="focal_ShowPostCount" <?php echo ($this->widgetOptions['ShowPostCount']) ? "checked" : ''; ?>/>&nbsp;<label
                                    for="focal_ShowPostCount"><?php _e('Show post count', 'focal'); ?></label><br/>
                                <input type="checkbox" id="focal_ShowPostCountUnique"
                                       name="focal_ShowPostCountUnique" <?php echo ($this->widgetOptions['ShowPostCountUnique']) ? "checked" : ''; ?>/>&nbsp;<label
                                    for="focal_ShowPostCountUnique"><?php _e('Show unique posts only', 'focal'); ?></label><br/>
                                ...
                                <br/>
                                <label
                                    for="focal_widegtCountBefore"><?php _e('Before post count:', 'focal'); ?> </label><input
                                    type="text" name="focal_widegtCountBefore" id="focal_widegtCountBefore"
                                    size="20" value="<?php echo $this->widgetOptions['CountBefore'] ?>"/><br/>
                                <label
                                    for="focal_widegtCountAfter"><?php _e('After post count:', 'focal'); ?> </label><input
                                    type="text" name="focal_widegtCountAfter" id="focal_widegtAfterBefore" size="20"
                                    value="<?php echo $this->widgetOptions['CountAfter'] ?>"/><br/>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="checkbox" id="focal_UseDescriptionAsCategoryText"
                                       name="focal_UseDescriptionAsCategoryText" <?php echo ($this->widgetOptions['UseDescriptionAsCategoryText']) ? "checked" : ''; ?>/>&nbsp;<label
                                    for="focal_UseDescriptionAsCategoryText"><?php _e('Use category description as the link text.', 'focal'); ?></label><br/>
                                <input type="checkbox" id="focal_UseDescriptionAsCategoryTitle"
                                       name="focal_UseDescriptionAsCategoryTitle" <?php echo ($this->widgetOptions['UseDescriptionAsCategoryTitle']) ? "checked" : ''; ?>/>&nbsp;<label
                                    for="focal_UseDescriptionAsCategoryTitle"><?php _e('Use category description as the popup (title) text.', 'focal'); ?></label><br/>
                                ...
                                <input type="checkbox" id="focal_DontLinkCurrentCategory"
                                       name="focal_DontLinkCurrentCategory" <?php echo ($this->widgetOptions['DontLinkCurrentCategory']) ? "checked" : ''; ?>/>&nbsp;<label
                                    for="focal_DontLinkCurrentCategory"><?php _e('Don\'t add link for current (selected) category', 'focal'); ?></label><br/>
                                <input type="checkbox" id="focal_NoFollowAllLinks"
                                       name="focal_NoFollowAllLinks" <?php echo ($this->widgetOptions['NoFollowAllLinks']) ? "checked" : ''; ?>/>&nbsp;<label
                                    for="focal_NoFollowAllLinks"><?php _e('Add rel="nofollow" to all links', 'focal'); ?></label><br/>
                                <input type="checkbox" id="focal_NoFollowParents"
                                       name="focal_NoFollowParents" <?php echo ($this->widgetOptions['NoFollowParents']) ? "checked" : ''; ?>/>&nbsp;<label
                                    for="focal_NoFollowParents"><?php _e('Add rel="nofollow" to all links NOT below current category (allow follow on child category)', 'focal'); ?></label><br/>
                            </td>
                        </tr>
                        <tr>
      				...
      				<span><?php _e('Text before and after the links are not included as the anchor tag, so you can have "Visit <span style="text-decoration:underline">my photos</span> page".</span>', 'focal'); ?></br>
                        ...
                        <p><span><?php _e('Links before the category list:', 'focal'); ?></span><br/>
                            <textarea name="focal_widgetLinksBefore" id="focal_widgetLinksBefore"
                                      style="width:80%; height:80px"><?php echo($this->widgetOptions['LinksBefore']); ?></textarea>
                        </p>
              <p><span><?php _e('Links after the category list:', 'focal'); ?></span><br/>
                  <textarea name="focal_widgetLinksAfter" id="focal_widgetLinksAfter"
                            style="width:80%; height:80px"><?php echo($this->widgetOptions['LinksAfter']); ?></textarea>
              </p>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <input type="checkbox" id="focal_OutputCatIDs"
                                       name="focal_OutputCatIDs" <?php echo ($this->widgetOptions['OutputCatIDs']) ? "checked" : ''; ?>/>&nbsp;<label
                                    for="focal_OutputCatIDs"><?php _e('Use category ID as CSS ID on &lt;li&gt; tags, e.g. &lt;li id=&quot;cat-53&quot;&gt;', 'focal'); ?></label><br/>
                                <?php
                                if ($this->widgetOptions['ApplyCSSTo'] == 'li') {
                                    $li = ' selected';
                                    $value = 'li';
                                ...
                                <select name="focal_applyCSSTo" id="focal_applyCSSTo" value="<?php echo $value; ?>">
                                    <option<?php echo $li; ?>
                                        value="li"><?php _e('List item tags (li) (default)', 'focal'); ?></option>
                                    <option<?php echo $a; ?>
                                        value="a"><?php _e('Links/Anchor tags (a)', 'focal'); ?></option>
                                    <option<?php echo $span; ?>
                                        value="span"><?php _e('Wrap contents in a span tag', 'focal'); ?></option>
                                </select>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <input type="checkbox" id="focal_Superfish"
                                       name="focal_Superfish" <?php echo ($this->widgetOptions['Superfish']) ? "checked" : ''; ?>/>&nbsp;<label
                                    for="focal_Superfish"><?php _e('<a href="http://users.tpg.com.au/j_birch/plugins/superfish/">Superfish Menu</a> support', 'focal'); ?></label><br/>
                                &nbsp; &nbsp; <input type="checkbox" id="focal_SuperfishJS"
                                                     name="focal_SuperfishJS" <?php echo ($this->defaultOptions['IncludeSuperfishJS']) ? "checked" : ''; ?>/>&nbsp;<label
                                    for="focal_SuperfishJS"><?php _e('Include Javascript (optional, will work with just CSS)', 'focal'); ?></label><br/>
                                &nbsp; &nbsp; <input type="checkbox" id="focal_SuperfishCSS"
                                                     name="focal_SuperfishCSS" <?php echo ($this->defaultOptions['IncludeSuperfishCSS']) ? "checked" : ''; ?>/>&nbsp;<label
                                    for="focal_SuperfishCSS"><?php _e('Include CSS (default only, you will need to copy CSS to your themes stylesheet to customise. Please refer to the <a href="http://www.lonewolfdesigns.co.uk/focal/" rel="nofollow">plugin homepage</a> for details.', 'focal'); ?></label><br/><br/>

                                <input type="checkbox" id="focal_Accordian"
                                       name="focal_Accordian" <?php echo ($this->widgetOptions['Accordian']) ? "checked" : ''; ?>/>&nbsp;<label
                                    for="focal_Accordian"><?php _e('<a href="http://www.i-marco.nl/weblog/jquery-accordion-3/">jQuery Accordian</a> support', 'focal'); ?></label><br/>
                                &nbsp; &nbsp; <input type="checkbox" id="focal_AccordianJS"
                                                     name="focal_AccordianJS" <?php echo ($this->defaultOptions['IncludeAccordianJS']) ? "checked" : ''; ?>/>&nbsp;<label
                                    for="focal_AccordianJS"><?php _e('Include Javascript', 'focal'); ?></label><br/>
                                &nbsp; &nbsp; <input type="checkbox" id="focal_AccordianCSS"
                                                     name="focal_AccordianCSS" <?php echo ($this->defaultOptions['IncludeAccordianCSS']) ? "checked" : ''; ?>/>&nbsp;<label
                                    for="focal_AccordianCSS"><?php _e('Include CSS (default only, you will need to copy CSS to your themes stylesheet to customise. Please refer to the <a href="http://www.lonewolfdesigns.co.uk/focal/" rel="nofollow">plugin homepage</a> for details.', 'focal'); ?></label><br/><br/>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row" valign="top"><?php _e('Miscellaneous', 'focal'); ?></th>
                            <td>
                                <input type="checkbox" id="focal_ShowEmptyCats"
                                       name="focal_ShowEmptyCats" <?php echo ($this->widgetOptions['ShowEmptyCats']) ? "checked" : ''; ?>/>&nbsp;<label
                                    for="focal_ShowEmptyCats"><?php _e('Show empty categories.', 'focal'); ?></label><br/>
                                <input type="checkbox" id="focal_MultiLingual"
                                       name="focal_MultiLingual" <?php echo ($this->widgetOptions['MultiLingual']) ? "checked" : ''; ?>/>&nbsp;<label
                                    for="focal_MultiLingual"><?php _e('Allow Multilingual Translations.', 'focal'); ?></label><br/>
                            </td>
                        </tr>
                    </table>
                    <br/>
