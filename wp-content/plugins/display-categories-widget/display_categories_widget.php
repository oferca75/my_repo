...
class DisplayCategoriesWidget extends WP_Widget
{
    ...
    function form($instance)
    {
        ...
        <p>
            ...
            <select id="<?php echo $this->get_field_id('category'); ?>" class="widefat"
                    name="<?php echo $this->get_field_name('category'); ?>">
                <?php echo implode('', $cat_options); ?>
            </select>
        </p>
        <p><label for="<?php echo $this->get_field_id('dcw_limit'); ?>"><?php _e('Limit (optional):'); ?> <input
                    class="widefat" id="<?php echo $this->get_field_id('dcw_limit'); ?>"
                    name="<?php echo $this->get_field_name('dcw_limit'); ?>" type="text"
                    value="<?php echo esc_attr($dcw_limit); ?>"/></label></p>
        <p>
            <label
                for="<?php echo $this->get_field_id('dcw_option_name'); ?>"><?php _e('Label no-selection (optional):'); ?>
                <input class="widefat" id="<?php echo $this->get_field_id('dcw_option_name'); ?>"
                       name="<?php echo $this->get_field_name('dcw_option_name'); ?>" type="text"
                       value="<?php echo esc_attr($dcw_option_name); ?>"/></label></p>
        <p>
            <label
                for="<?php echo $this->get_field_id('dcw_exclude'); ?>"><?php _e('Category ID\'s to exclude (optional):'); ?>
                <input class="widefat" id="<?php echo $this->get_field_id('dcw_exclude'); ?>"
                       name="<?php echo $this->get_field_name('dcw_exclude'); ?>" type="text"
                       value="<?php echo esc_attr($dcw_exclude); ?>"/></label><br><?php _e('<small>Ex: 26,32,54 (comma-separated list of category ids)</small>'); ?>
        </p>
        <p>
            <label
                for="<?php echo $this->get_field_id('dcw_depth'); ?>"><?php _e('Levels in the hierarchy to show (optional):'); ?>
                <input class="widefat" id="<?php echo $this->get_field_id('dcw_depth'); ?>"
                       name="<?php echo $this->get_field_name('dcw_depth'); ?>" type="text"
                       value="<?php echo esc_attr($dcw_depth); ?>"/></label><br><?php _e('<small>0 - All Categories and child Categories (Default).<br>
...
n - Value of n (some number) specifies the depth (or level) to descend in displaying Categories</small>'); ?></p>
        <p><?php _e('Display Parent? (optional):'); ?> <br><input
                name="<?php echo $this->get_field_name('display_parent'); ?>" type="radio"
                value="1" <?php if (esc_attr($display_parent) == 1) echo "checked"; ?> />Yes &nbsp; <input
                name="<?php echo $this->get_field_name('display_parent'); ?>" type="radio"
                value="0" <?php if (esc_attr($display_parent) == 0) echo "checked"; ?>/>No </p>

        <p><?php _e('Show? (optional):'); ?> <br><input name="<?php echo $this->get_field_name('show_format'); ?>"
                                                        type="radio"
                                                        value="0" <?php if (esc_attr($show_format) == 0) echo "checked"; ?> />List
            &nbsp; <input name="<?php echo $this->get_field_name('show_format'); ?>" type="radio"
                          value="2" <?php if (esc_attr($show_format) == 2) echo "checked"; ?>/>Drop Down </p>

        <p><?php _e('Display Empty categories? (optional):'); ?> <br><input
                name="<?php echo $this->get_field_name('display_empty_categories'); ?>" type="radio"
                value="0" <?php if (esc_attr($display_empty_categories) == 0) echo "checked"; ?> />Yes &nbsp;
            <input name="<?php echo $this->get_field_name('display_empty_categories'); ?>" type="radio"
                   value="1" <?php if (esc_attr($display_empty_categories) == 1) echo "checked"; ?>/>No </p>
        <p><?php _e('Display Number of posts near categories? (optional):'); ?> <br><input
                name="<?php echo $this->get_field_name('showcount_value'); ?>" type="radio"
                value="1" <?php if (esc_attr($showcount_value) == 1) echo "checked"; ?> />Yes &nbsp; <input
                name="<?php echo $this->get_field_name('showcount_value'); ?>" type="radio"
                value="0" <?php if (esc_attr($showcount_value) == 0) echo "checked"; ?>/>No </p>

        <p>
            <label for="<?php echo $this->get_field_id('dcw_column'); ?>">
                <?php _e('No. of columns to display (optional):'); ?>
            </label>
            <select id="<?php echo $this->get_field_id('dcw_column'); ?>" class="widefat"
                    name="<?php echo $this->get_field_name('dcw_column'); ?>">
                <?php for ($dcw_i = 1; $dcw_i < 4; $dcw_i++) {
                    ?>
                    <option
                        value=<?php echo $dcw_i; ?> <?php if ($instance['dcw_column'] == $dcw_i) echo ' selected="selected"'; ?>><?php echo $dcw_i; ?></option>
                <?php } ?>
            </select>
        </p>
        <?php
