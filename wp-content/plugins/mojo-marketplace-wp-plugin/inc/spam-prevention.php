        ...
        if (!is_wp_error($result)) {
            ...
            if ($data = json_decode($result['body'])) {
                ...
                if (isset($data->ip) || isset($data->email) || isset($data->username)) {
                    ...
                    if (isset($data->ip->confidence) && $data->ip->confidence > get_option('mm_confidence_ip', 75)) {
                        $blocked = 'ip';
                    }
                    if (isset($data->username->confidence) && $data->username->confidence > get_option('mm_confidence_username', 95)) {
                        $blocked = 'username';
                    }
                    if (isset($data->email->confidence) && $data->email->confidence > get_option('mm_confidence_email', 75)) {
                        $blocked = 'email';
                    }
