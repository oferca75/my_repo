...
class Minify_Cache_APC
{
    ...
    public function store($id, $data)
    {
        ...
        return apc_store($id . '_' . $this->_blog_id, serialize($v), $this->_exp);
    }
    ...
    private function _get_key_version()
    {
        if ($this->_key_version <= 0) {
            $v = apc_fetch($this->_get_key_version_key());
            $v = intval($v);
    ...
    private function _fetch($id)
    {
        ...
        $v = @unserialize(apc_fetch($id . '_' . $this->_blog_id));
        ...
        if ($expires_at == null || time() > $expires_at) {
            ...
            apc_store($id . '_' . $this->_blog_id, serialize($v), 0);
            $this->_id = null;
            return false;
        }
    ...
    {
        apc_store($this->_get_key_version_key(), $v, 0);
    }
