...
';
        ...
        <p><label
                for="<?php echo $this->get_field_id('days'); ?>"><?php esc_html_e('Number of days to show:', 'better-analytics'); ?></label>
            <input id="<?php echo $this->get_field_id('days'); ?>" name="<?php echo $this->get_field_name('days'); ?>"
                   type="text" value="<?php echo $days; ?>" min="1" style="width:80px;"/></p>

        <p><label
                for="<?php echo $this->get_field_id('format'); ?>"><?php esc_html_e('HTML Format For Widget:', 'better-analytics'); ?></label>
            <textarea id="<?php echo $this->get_field_id('format'); ?>"
                      name="<?php echo $this->get_field_name('format'); ?>" type="text"
                      style="display:block;width:100%"><?php echo esc_textarea($format); ?></textarea>
            <span
                class="description"><?php esc_html_e('You can add HTML formatting here if you wish.  %1$s = metric name, %2$s = value', 'better-analytics'); ?></span>
        </p>

        <p>
            <label for="<?php echo $this->get_field_id('this_page_only'); ?>">
                <input id="<?php echo $this->get_field_id('this_page_only'); ?>"
                       name="<?php echo $this->get_field_name('this_page_only'); ?>" type="checkbox"
                       value="1" <?php checked('1', $thisPageOnly); ?>>
                <?php esc_html_e('Stats for current page only', 'better-analytics'); ?></label>
        </p>

        <p>
            <label for="<?php echo $this->get_field_id('private'); ?>">
                <input id="<?php echo $this->get_field_id('private'); ?>"
                       name="<?php echo $this->get_field_name('private'); ?>" type="checkbox"
                       value="1" <?php checked('1', $private); ?>>
