...
if (!class_exists('WPCTC_Module')) {
    ...
    abstract class WPCTC_Module
    {
         ...
         * @mvc @model
         *
         * @param bool|string $default_template_path The path to the template, relative to the plugin's `views` folder
        ...
        {
            do_action('wpctc_render_template_pre', $default_template_path, $variables);

            $template_path = locate_template(basename($default_template_path));
            if (!$template_path) {
                $template_path = dirname(__DIR__) . '/views/' . $default_template_path;
            }
            $template_path = apply_filters('wpctc_template_path', $template_path);
            ...
            if (is_file($template_path)) {
                ...
                $template_content = apply_filters('wpctc_template_content', ob_get_clean(), $default_template_path, $template_path, $variables);
            } else {
                $template_content = '';
            }

            do_action('wpctc_render_template_post', $default_template_path, $variables, $template_path, $template_content);
