...
<form action="<?php echo W3TC_PAYPAL_URL; ?>" method="get">
    <div class="metabox-holder">
        ...
        <p>
            <strong><?php echo sprintf(__('Price: %.2f USD', 'w3-total-cache'), $this->_request_prices[$request_type]) ?></strong>
        </p>
        ...
        <p>
            ...
            <input type="hidden" name="item_name"
                   value="<?php echo esc_attr(sprintf('%s: %s (#%s)', ucfirst(w3_get_host()), $this->_request_types[$request_type], $request_id)); ?>"/>
            <input type="hidden" name="amount"
                   value="<?php echo sprintf('%.2f', $this->_request_prices[$request_type]); ?>"/>
            ...
            <input id="support_cancel" class="{nonce: '<?php echo wp_create_nonce('w3tc'); ?>'}" type="button"
                   value="<?php _e('Cancel', 'w3-total-cache'); ?>" class="button-primary"/>
        </p>
