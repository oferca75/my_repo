...
function wp_supercache_awaitingmoderation_admin()
{
    ...
    <fieldset id="<?php echo $id; ?>" class="options">
        ...
        <form name="wp_manager" action="" method="post">
            <label><input type="radio" name="cache_awaitingmoderation" value="1" <?php if ($cache_awaitingmoderation) {
                    echo 'checked="checked" ';
                } ?>/> <?php _e('Enabled', 'wp-super-cache'); ?></label>
            <label><input type="radio" name="cache_awaitingmoderation" value="0" <?php if (!$cache_awaitingmoderation) {
                    echo 'checked="checked" ';
